...
| `AUDIO_ENCODING` / `AUDIO_SAMPLE_RATE` | `linear16` / `8000` | Audioformat Richtung Voice-Provider (kein Transcoding engine-seitig). 16-kHz-Pipeline *(0.6.24)*: `16000` setzen — zusammen mit `EXTERNAL_MEDIA_FORMAT=slin16`; HD-Audio Ende-zu-Ende im Web-Widget, zum G.711-Trunk transkodiert Asterisk (bleibt 8 kHz); Bridge-Aufnahmen dann als `wav16`. |
