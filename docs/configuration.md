...
| `NATIVE_STT_LANGUAGE_LOCK` *(0.15.0)* | `true` | **Sprachnachführung.** Meldet Flux über zwei aufeinanderfolgende *lange* Turns (≥ 25 Zeichen) eine andere Sprache als eingestellt, wird der Hinweis per `Configure` mitten im Strom nachgezogen — ohne Neuverbindung. Kurze Turns zählen bewusst nicht: Genau dort ist die Erkennung unzuverlässig, und eine Umschaltung darauf würde den Fehler verstärken. Nur bei `flux-general-multi` wirksam. |
