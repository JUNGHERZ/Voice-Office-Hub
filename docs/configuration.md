...
| `SPEECHIFY_API_KEY` *(0.8.4)* | *(leer)* | Speechify Simba (`speak.provider: "speechify"`, nur native Kaskade). Stimm-ID in `speak.voice`, Modell in `speak.model` — Default `simba-3.0`, denn **nur 3.0 kann Deutsch**. 🟡 US-Verarbeitung. |
| `FISH_AUDIO_API_KEY` / `FISH_AUDIO_ENABLED` *(0.8.4)* | *(leer)* / `false` | Fish Audio S2 (`speak.provider: "fish_audio"`, nur native Kaskade). 🔴 **Drittland** (China, kein Angemessenheitsbeschluss) — ohne `FISH_AUDIO_ENABLED=true` weder im Panel noch im Anruf verfügbar. Abrechnung in UTF-8-**Bytes**. |
| `DEEPGRAM_REGION` *(0.8.12)* | `global` | Verarbeitungsregion aller vier Deepgram-Dienste: `global` (USA) oder `eu` (`api.eu.deepgram.com`, Frankfurt) — gleicher Key, nur andere Domain. Setzt die Defaults für `NATIVE_STT_URL`, `NATIVE_TTS_URL`, `NATIVE_TTS_FLUX_URL` und `DEEPGRAM_AGENT_URL` gemeinsam; einzeln gesetzte URLs gewinnen weiterhin. **Für Installationen in Europa dringend empfohlen** — ohne sie geht das Anruferaudio zur Erkennung in die USA (Drittlandsübermittlung), und der Verbindungsaufbau kostet aus Deutschland rund 460 ms mehr pro Anruf (gemessen: 514 ms gegen 52 ms bis zur offenen Flux-Verbindung). Ein unbekannter Wert fällt auf `global` zurück und wird beim Start als Fehler protokolliert; laufen Region und einzelne URLs auseinander, warnt die Engine mit den Namen der Abweichler. Die Voice-Agent-URL ist der Sonderfall: global liegt sie auf `agent.deepgram.com`, in der EU auf `api.eu.deepgram.com` — ein `agent.eu.deepgram.com` existiert nicht. |
...
| `NATIVE_STT_LANGUAGE_LOCK` *(0.15.0, Verhalten geändert in 0.17.0)* | `true` | **Erkennungssprache nachziehen.** Sobald der Localizer die Gesprächssprache autoritativ bestimmt hat (LLM über das Gesprächsfenster), wird der Flux-Hinweis per `Configure` mitten im Strom nachgezogen — ohne Neuverbindung. Bewusst diese Richtung: Ein gesetzter Hinweis verbiegt die Erkennung selbst, ein Regelkreis auf ihr Signal bestätigt nur die eigene Vorgabe. Nur bei `flux-general-multi` wirksam. |
| `NATIVE_DUPLEX_ENABLED` *(0.13.0)* | `true` | **Betriebs-Not-Aus** für den experimentellen Duplex-Pfad. Freigeschaltet wird er pro Agent über `voiceProvider: duplex`; diese Variable stellt im Störfall alle Duplex-Agenten auf einen Schlag auf die native Kaskade zurück, ohne dass Agenten bearbeitet werden müssen. Der Anruf läuft dann normal weiter, im Log steht eine Warnung. |
| `NATIVE_LOG_TURN_UPDATES` *(0.12.2)* | `false` | Reiner **Messschalter**: protokolliert Flux' `Update`-Ereignisse (etwa alle 0,25 s je Anruf) mit Turn-Ende-Konfidenz, Zeichenzahl und den letzten 48 Zeichen des kumulativen Transkripts. Bewusst auf `info`-Pegel, damit eine Live-Instanz für die Messung nicht auf `debug` gehoben werden muss. Der native Pfad wertet die Ereignisse nicht aus — sie sind die Auslegungsgrundlage für eine Gesprächssteuerung, die entscheidet, bevor der Turn zu Ende ist. Für den Normalbetrieb ausgeschaltet lassen. |
...
| `listen.keyterms[]` / `smart_format` | STT-Feintuning. `smart_format` gilt nur für nova-3 — der Settings-Builder sendet je Modell nur Gültiges. |
| *(abgeleitet)* `language_hints` *(0.14.0)* | **Kein Feld mehr.** Der Sprach-Hinweis für `flux-general-multi` wird aus `contentLanguage` abgeleitet (der Sprache von Begrüßung und Prompt): genau ein Hinweis, oder gar keiner, wenn die Sprache das Modell nicht kennt. Bis 0.13.x stand hier `["de", "en"]` als unsichtbare Vorgabe — damit war Flux gesagt, auch Englisch zu erwarten, und kurze deutsche Äußerungen kamen als englische Phantasiesätze zurück. |
...
| --- | --- |
| `listen.model` | Muss ein `flux-*`-Modell sein (modellintegriertes Turn-Taking); steht `nova-3`, fällt die Engine mit Warnung auf `flux-general-multi` zurück. `eot_threshold`/`eot_timeout_ms`/`keyterms` gehen 1:1 an Flux, der Sprach-Hinweis wird abgeleitet (s. o.); `smart_format` ist wirkungslos. |
...
| `speak.sanitize` *(0.11.2)* | Formatierung vor der Synthese entfernen. **Default an** (`SPEECH_SANITIZE`). Sprachmodelle schreiben, als säßen sie in einem Chatfenster — `**fett**`, Aufzählungsstriche, gelegentlich ein Emoji —, und eine Synthese liest das wörtlich vor („Sternchen Sternchen", „winkende Hand"). Entfernt werden Markdown-Auszeichnung, Überschriften, Listenmarker, Code-Zäune, Links (der Text bleibt, das Ziel geht) und Piktogramme; Zeilenumbrüche werden zu Sprechpausen. **Nur der Weg zur Synthese** ist betroffen: Ins Transkript geht weiterhin der Rohtext des Modells, damit Auswertungen und Zusammenfassungen darauf laufen können. Ein Hinweis im Prompt ersetzt das nicht — er ist eine Bitte, keine Zusicherung. |
...
Grenze der Textaufbereitung: Sie greift auf dem **nativen** Pfad, weil dort die Engine den Text an die Synthese gibt. Beim gebündelten Voice-Agent formuliert und spricht der Anbieter selbst — dort erreicht die Säuberung nur die Begrüßung (auch die erzeugte) und eingespielte Ansagen, nicht die Antworten des Anbieters. Ein Agent, dessen Modell zu Auszeichnung neigt, gehört deshalb auf den nativen Pfad.
