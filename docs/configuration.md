...
| `widget.enabled` / `exten` / `allowedOrigins` / `showTranscript` *(0.6.9)* | **Web-Widget** (einbettbares Browser-Softphone): `exten` = 3-stellige Pseudo-Durchwahl (muss auch in `targetNumbers` stehen), `allowedOrigins` = Websites, die einbetten dürfen (CSP frame-ancestors), `showTranscript` = Live-Transkript im Widget. Der Embed-`key` ist server-verwaltet (rotierbar). Setzt `WEBRTC_ENABLED=true` voraus — Details in [docs/webrtc.md](webrtc.md). |
