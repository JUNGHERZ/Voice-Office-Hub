# Tools: eingebaut + eigene HTTP-Endpoints pro Agent

Der Voice-Agent kann während des Gesprächs Funktionen aufrufen (Function Calling). Es gibt
zwei Quellen, die pro Anruf zu **einem Toolset** zusammengeführt werden
...
Kontrakt (Envelope, GET-Query, `${ENV:}`, Timeout- und Fehlerverhalten) gegen einen lokalen
HTTP-Server.
